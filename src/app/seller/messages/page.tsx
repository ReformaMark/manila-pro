...
import { MenuIcon, MessageSquare, Search } from "lucide-react";
import { Input } from "@/components/ui/input";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { cn, formatDateListed } from "@/lib/utils";
import { Badge } from "@/components/ui/badge";
import ChatArea from "@/app/messages/_components/chat-area";
...
const MessagesPage = () => {
  ...
  const getOtherUser = (receiverId: Id<"users">) => {
    const conversation = conversations?.find(
      (conversation) => conversation.receiver._id === receiverId
    );
    if (conversation) {
  ...
  useEffect(() => {
    ...
    const filtered = conversations.filter((conversation) => {
      ...
      return (
        fname.toLowerCase().includes(lowerSearch) ||
              ...
              <div className="w-full lg:w-1/3 border-r border-gray-200 flex flex-col">
                <div className="p-3 border-b border-gray-200">
                  <div className="relative">
                    <Input
                      onChange={(e) =>
                        setSearchConversationText(e.target.value)
                      }
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className="text-xs text-gray-500">
                                    {formatDateListed(
                                      conversation.messages[0]._creationTime
                                    )}
                                  </span>
                                </div>
            ...
            {selectedConversation ? (
              ...
              <div
                className={cn(
                  ...
                  "w-full md:w-2/3 min-h-[70vh]  items-center justify-center"
                )}
              >
