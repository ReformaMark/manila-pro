...
import { ReviewStepDialog } from "../property-form/review-step"

interface EditPropertyImagesFormProps {
...
export const EditPropertyImagesForm = ({
    form,
    displayImagePreview,
    otherImagePreviews,
    setDisplayImagePreview,
    setOtherImagePreviews,
}: EditPropertyImagesFormProps) => {
    const [isOpen, setIsOpen] = useState(false)
    const [isSingleImageUploading, setIsSingleImageUploading] = useState(false)
    const generateUploadUrl = useMutation(api.files.generateUploadUrl)

    useEffect(() => {
        const property = form.getValues()

        // Only set preview images on initial load
        const hasNoPreviewsSet = !displayImagePreview && !otherImagePreviews.length

        if (hasNoPreviewsSet) {
            // @ts-expect-error Preview image from backend
            if (property.previewImage) {
                // @ts-expect-error Preview image from backend
                setDisplayImagePreview(property.previewImage)
            }

            // @ts-expect-error Preview images from backend
            if (property.otherPreviewImages?.length) {
                // @ts-expect-error Preview images from backend
                setOtherImagePreviews(property.otherPreviewImages)
            }
        }
    }, [form])
    ...
    const handleOtherImagesUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const existingStorageIds = form.getValues("otherImage") || [];
            const newStorageIds = uploadResults.map(r => r.storageId);
            form.setValue("otherImage", [...existingStorageIds, ...newStorageIds]);
    ...
    const removeOtherImage = (index: number) => {
        ...
        form.setValue("otherImage", updatedImages)
    }
    ...
    useEffect(() => {
        return () => {
            otherImagePreviews.forEach(url => {
